...
export const meetupsRoutes = (db) => {
  ...
  meetupsRouter.post('/', async (req, res) => {
    try {
      const response = {
        ...
        modified: req.body.data.modified,
        start: req.body.data.start,
        finish: req.body.data.finish,
        author: {
          id: req.body.data.author.id,
          name: req.body.data.author.name,
          surname: req.body.data.author.surname,
        },
        speakers: req.body.data.speakers.map((s) => ({
          id: faker.datatype.uuid(),
          name: s.name,
          surname: s.surname,
        })),
        subject: req.body.data.subject,
        excerpt: req.body.data.excerpt,
        place: req.body.data.place,
        goCount: 0,
        status: 'REQUEST',
        isOver: false,
        image: req.body.data.image.url,
      };
      if (
        ...
        res
          .status(500)
          .send('Error. Dates must be valid and start date must be earlier than finish date!');
      }
    } catch (err) {
  ...
  meetupsRouter.put('/', async (req, res) => {
    const index = db.data.meetups.findIndex((it) => it.id === req.body.data.id);
    db.data.meetups[index] = { ...db.data.meetups[index], ...req.body.data };
    await db.write();
  ...
  meetupsRouter.get('/:id/participants', async (req, res) => {
    res.send(db.data.participants[req.params.id]);
  });

  meetupsRouter.post('/:id/participants', ensureAuthenticated, async (req, res) => {
    try {
  ...
  meetupsRouter.delete('/:id/participants', ensureAuthenticated, async (req, res) => {
    try {
  ...
  meetupsRouter.get('/:id/votedusers', async (req, res) => {
    res.send(db.data.votedUsers[req.params.id]);
  });

  meetupsRouter.post('/:id/votedusers', async (req, res) => {
    try {
  ...
  meetupsRouter.delete('/:id/votedusers', async (req, res) => {
    try {
